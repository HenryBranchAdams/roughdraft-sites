...
> = {
  changed: {
    ...
    body: "A collaborator saved a newer version. Reload the latest copy, or explicitly replace it with your current draft.",
  },
  conflict: {
    title: "Save conflict",
    body: "The shared document changed while you had unsaved edits. Autosave is paused so your draft cannot overwrite those changes.",
  },
  paused: {
    title: "Autosave paused",
    body: "Keep editing locally, then reload the shared document to discard your draft or explicitly replace it when you are ready.",
  },
};
...
export function DocumentWorkspace({
  ...
  const reviewHandoffStatusBody =
    reviewHandoffState === "undelivered"
      ? "The hosted review event could not be stored. Your document edits remain in the current draft."
      : reviewHandoffState === "error"
        ? "Roughdraft could not save the hosted review event. Try again after checking your connection."
        : null;
                        ...
                        {reviewHandoffStatusBody ? (
                          ...
                          <div className="mt-1">
                            <p className="text-sm leading-[1.32rem] text-stone-500 dark:text-slate-400">
                              Your review is saved in the shared workspace.
                              Export the Markdown when you want to hand it back
                              to Codex or another tool. You can also{" "}
                              <button
                                type="button"
                              ...
                              >
                                copy a handoff message
                              </button>{" "}
                              for a manual continuation.
                            </p>
                            <Button
                              type="button"
